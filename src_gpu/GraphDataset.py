...
def strict_metapath_random_walk(data, start_node, metapath, walk_length):
    """
    Perform a strict random walk on a PyTorch Geometric Data object where the next node type 
    must be the exact next type in the metapath after the current node's type.
    ...
    cur_node = start_node
    cur_type_index = metapath.index(data.node_type[cur_node])
    
    while len(walk) < walk_length:
        cur_type = data.node_type[cur_node]
        
        # Determine the next type strictly from the metapath
        next_type_index = (cur_type_index + 1) % len(metapath)
        valid_next_type = metapath[next_type_index]
        
        # Find neighbors and filter by the exact next type
        neighbors = (data.edge_index[0] == cur_node).nonzero(as_tuple=True)[0]
        neighbors = data.edge_index[1][neighbors].tolist()
        next_candidates = [n for n in neighbors if data.node_type[n] == valid_next_type]
        ...
        cur_node = next_node
        cur_type_index = metapath.index(data.node_type[cur_node])  # Update current type index for the next step
